...
__device__ float image3_bilinear_interpolate(
    const float *pixels,
    int width,
    int height,
    float u,
...
__global__ void image3_bilinear_scale_kernel(
    const float *in_pixels,
    int in_width,
    int in_height,
    int channels,
    ...
    int out_height)
{
  ...
  if ((x < out_width) && (y < out_height) && (c < channels)) {
    ...
    float interp_value = image3_bilinear_interpolate(in_pixels, in_width, in_height, u, v, c);

    out_pixels[x + y * out_width + c * out_width * out_height] = interp_value;
  }
}

extern "C" void rembrandt_kernel_image3_bilinear_scale(
    const float *in_pixels,
    int in_width,
    int in_height,
    int channels,
...
{
  int n = out_width * out_height * channels;
  image3_bilinear_scale_kernel<<<(n+1024-1)/1024, 1024, 0, stream>>>(
      in_pixels,
      in_width,
      in_height,
      channels,
