...
__device__ float tex2d_clamp(const float *pixels, int width, int height, int u, int v, int c) {
    ...
    int c)
{
  ...
  return interpolate_bicubic_filter(fy,
      interpolate_bicubic_filter(fx,
          tex2d_clamp(pixels, width, height, ipx-1, ipy-1, c),
          tex2d_clamp(pixels, width, height, ipx,   ipy-1, c),
          tex2d_clamp(pixels, width, height, ipx+1, ipy-1, c),
          tex2d_clamp(pixels, width, height, ipx+2, ipy-1, c)),
      interpolate_bicubic_filter(fx,
          tex2d_clamp(pixels, width, height, ipx-1, ipy,   c),
          tex2d_clamp(pixels, width, height, ipx,   ipy,   c),
          tex2d_clamp(pixels, width, height, ipx+1, ipy,   c),
          tex2d_clamp(pixels, width, height, ipx+2, ipy,   c)),
      interpolate_bicubic_filter(fx,
          tex2d_clamp(pixels, width, height, ipx-1, ipy+1, c),
          tex2d_clamp(pixels, width, height, ipx,   ipy+1, c),
          tex2d_clamp(pixels, width, height, ipx+1, ipy+1, c),
          tex2d_clamp(pixels, width, height, ipx+2, ipy+1, c)),
      interpolate_bicubic_filter(fx,
          tex2d_clamp(pixels, width, height, ipx-1, ipy+2, c),
          tex2d_clamp(pixels, width, height, ipx,   ipy+2, c),
          tex2d_clamp(pixels, width, height, ipx+1, ipy+2, c),
          tex2d_clamp(pixels, width, height, ipx+2, ipy+2, c)));
}
...
extern "C" void rembrandt_kernel_interpolate_bicubic(
    const float *in_pixels,
    int in_width,
    int in_height,
    int channels,
...
extern "C" void rembrandt_kernel_interpolate_catmullrom(
    const float *in_pixels,
    int in_width,
    int in_height,
    int channels,
...
extern "C" void rembrandt_kernel_interpolate_2x2_bilinear(
    const float *in_pixels,
    int in_width,
    int in_height,
    int channels,
